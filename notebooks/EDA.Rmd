...
matriz_confusion3 <- table(m3$model$y, predicciones3,
                          dnn = c("observaciones", "predicciones"))
matriz_confusion3
```
```{r}
TN1 <- matriz_confusion3[1,1]
FP1 <- matriz_confusion3[1,2]
FN1 <- matriz_confusion3[2,1]
TP1 <- matriz_confusion3[2,2]
```
...
tabla2 <- matrix(data = c(precision1,recall1,F11,accuracy1), ncol=1, nrow=4)
...
```{r}
AIC1 <- c(AIC(m1), AIC(m3))
BIC1 <- c(BIC(m1), BIC(m3))
R21 <- c(R2.hl, R2.hl1)
tabla2 <- matrix(data = c(AIC1,BIC1,R21), ncol=3, nrow=2)
colnames(tabla2) <- c("AIC","BIC","R2")
rownames(tabla2) <- c("Modelo 1: completo", "Modelo 3: variables sign.")
tabla2
```
