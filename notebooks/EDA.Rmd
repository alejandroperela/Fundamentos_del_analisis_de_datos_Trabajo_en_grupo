...
Antes de continuar con el análisis, vamos a analizar en mayor profundidad la relación entre distantas características de los registros

También intentaremos analizar por qué hay determinadas variables que son tan relevantes, sin que haya una lógica aparente detrás de ello, como el dispositivo de contacto o el mes de contacto. Y 
trataremos de averiguar si alguna variable no es relevante por sí sola, si no que depende de otras variables, y poder así reducir variables redundantes
...
Podemos ver que entre los 18 y los 30, el estado civil predominante es el de soltero, mientras que el de casado es el más présente a partir de los 30
...
ggplot(data,aes(x=month, fill=job)) + 
  geom_bar() + 
  ggtitle ("Marzo, Spetiembre, Octubre y Diciembre: los meses con MEJOR ratio de conversión") + 
  xlab("Edad") + 
  ylab("Trabajo") + 
  labs(fill="y") + 
  scale_x_discrete(limits = positions)
...
```{r na_visualization, warning=FALSE, message=FALSE}
...
Viendo las gráficas de densidad y el gráfico de líneas, no parece que haya una relación lógica entre ambas variables, pero viendo el boxplot nos damos cuenta de que como antes, cuantos 
mejor es el indicador macro, menor es la cantidad de gente que contrata el producto
