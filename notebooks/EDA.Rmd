...
```
A pesar de que graficamente no se observan diferencias significativas, al realizar el test chi-cuadrado rechazamos la hipotesis nula, por lo que sí hay una asociacion significativa entre la varibale housing y la clase y
...
chisq.test(tabla)
```
Rechazamos la hipotesis nula para cualquier nivel de significancia. La variable contact presenta una asociacion estadisticamente signficativa para y
...
chisq.test(tabla)
```
Rechazamos la hipotesis nula para cualquier nivel de significancia, es decir, existe una asociacion significativa entre el dia de la semana y la clase y
...
chisq.test(tabla)
```
Rechazamos la hipotesis nula. Por lo tanto, existe una asociacion estadisticamente significativa entre la variable pdays_aux y la clase y
...
t.test(age~y)
```
Observando el p-valor sí se consideraría que existen diferencias significativas entre los grupos, por tanto, la edad sí esta asociado a y
...
t.test(duration~y)
```
Sí existe diferencia significativa entre los grupos, por tanto, la duracion de la ultima llamada sí esta asociado a y
...
t.test(campaign~y)
```
Rechazamos la hipotesis nula para cualquier nivel de significancia por lo que sí se consideraría que existe diferencia significativa entre los grupos. Por lo tanto, campaign sí esta asociado a y
...
```
El p-valor es inferior a 0.05, rechazamos la hipotesis nula al considerar que sí existe diferencia significativa entre los grupos, es decir, el ratio de variacion del empleo sí esta asociado a y
...
```
Se consideraría que sí existe diferencia significativa entre los grupos al rechazar la hipotesis nula, por tanto, el indice de precios al consumo sí esta asociado a y
...
```
Al rechazar la hipotesis nula se considera que sí existe diferencia significativa entre los grupos, por tanto, existe una asociacion estadisticamente significativa entre el indice de confianza del consumidor y la clase y
...
t.test(euribor3m~y)
```
Rechazamos la hipotesis nula para cualquier nivel de significancia, se considera que sí existe diferencia significativa entre los grupos, por tanto, el euribor sí esta asociado a y
...
Ambos métodos sugieren como variables relevantes comunes el mes, la duración de la llamada y el poutcome.
...
m4 <- glm(y ~ month+duration+poutcome, data = data_train, family = "binomial")
m5 <- glm(y ~ month+poutcome, data = data_train, family = "binomial")
```

El modelo 0 corresponde al modelo sin variables.
El modelo 1 corresponde al modelo con todas las variables.
Los modelos 2 y 3 son modelos con la variable que menos correlaciona (edad) y la que más correlaciona (tasa de variación de empleo).
El modelo 4 corresponde a las variables comunes entre los métodos automáticos de forward y backward.
El modelo 5 es igual que el modelo 4, sin la variable duración de la llamada.
...
```{r}
compare_performance(m0,m1,m3,m4,m5,m6)
``` 

Los modelos muestran valores altos de AIC y BIC en general, siendo índices de ajuste de parsimonia, sancionando modelos que requieren un mayor número de variables. Los menores valores son del modelo 1, pero incluye la variable duración, una variable que no será útil puesto que la obtenemos tras la realización de la llamada. El modelo 5 está sin dicha variable, obteniendo valores ligeramente más bajos de $R^2$, y de RMSE, que indica el error cuadrático medio, siendo un índice que cuanto más bajo será mejor. 

```{r}
plot(compare_performance(m0,m1,m2,m3,m4,m5,m6, rank = TRUE))
```
Gráficamente, podemos evaluar que cuanto más cercano del centro, mejor rendimiento muestra.
...
## Modelo final seleccionado. Modelo...
