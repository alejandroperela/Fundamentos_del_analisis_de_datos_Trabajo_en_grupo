...
impdata_train <- mice(data_train, m=5, maxit = 50, method = 'cart', seed = 1234)
summary(impdata_train)
sum(is.na(impdata_train))
```
En estimaciones de valores perdidos, dentro de las estimaciones univariantes, usaremos el método de árbol de decisiones, frente a otros métodos de regresión o la media y la moda.
...
Graficamente se observan diferencias significativas entre los  grupos, para comprobar si es cierto que hay diferencias significativas vamos a hacer un test chi-cuadrado
```{r}
tabla<-table(y,job)
chisq.test(tabla)
```
Rechazamos la hipótesis nula, luego consideraremos que no hay independencias entre ambas variables, es decir, el trabajo sí está asociado a y
...
chisq.test(tabla)
```
Rechazamos la hipótesis nula, luego consideraremos que no hay independencias entre ambas variables, es decir, el estado civil sí está asociado a y
...
```
Rechazamos la hipótesis nula, luego consideraremos que no hay independencias entre ambas variables, es decir, la educacion sí está asociado a y
...
fisher.test(tabla)
```
No podemos rechazar la hipotesis nula, por tanto las variables si son independientes
...
```
Rechazamos H0, housing asociado a y
...
chisq.test(tabla)
```
NO rechazamos H0, loan NO asociado a y
...
chisq.test(tabla)
```
Rechazamos H0, contact asociado a y
...
chisq.test(tabla)
```
Rechazamos H0, month asociado a y
...
chisq.test(tabla)
```
Rechazamos H0, day of week asociado a y
...
Observamos que el ratio de conversión más alto es el que corresponde a la gente que nunca fue contactada en la campaña anterior, por lo que Dias desde el contacto previo puede ser un buen predictor
 ...
 para comprobar si es cierto que hay diferencias significativas vamos a hacer un test chi-cuadrado
```{r}
tabla<-table(y,pdays_aux)
chisq.test(tabla)
```
Rechazamos H0, pdays_aux asociado a y
...
chisq.test(tabla)
```
Rechazamos H0, poutcome asociado a y
...
t.test(age~y)
```
se consideraría que sí existe diferencia significativa entre los grupos, por tanto, la edad sí esta asociado a y
...
t.test(duration~y)
```
p-value<0.05 --> se consideraría que sí existe diferencia significativa entre los grupos, por tanto, la duracion de la ultima llamada sí esta asociado a y
...
t.test(campaign~y)
```
p-value<0.05 -> se consideraría que sí existe diferencia significativa entre los grupos, por tanto, sí esta asociado a y
...
*1. Normalidad*
```{r}
#mvn(data=data_trainnum$age, univariatePlot = "qqplot")
```
*2. Linealidad*
...
m3 <- glm(y ~age, data = data_train, family = "binomial")
m4 <- glm(y ~emp.var.rate, data = data_train, family = "binomial")
```
El modelo 0 corresponde al modelo sin variables y el modelo 1 incluye todas las variables.
El modelo 2 recoge el método stepwise, forward selection, añadiendo cada vez más variables evaluando el AIC en cada paso. 
Los modelos 3 y 4 son modelos con la variable que menos correlaciona (edad) y la que mas correlaciona (tasa de variación de empleo)
...
```
Hay presencia de autocorrelación.
...
```{r}
m5 <- glm(y ~ job+education+default+contact+month+campaign+pdays+poutcome+emp.var.rate+cons.price.idx+cons.conf.idx+nr.employed, family = binomial, data= data_train)
summary(m5)
```
```{r}
vif(m5)
dwt(m5, alternative = "two.sided")
```
Hay presencia de autocorrelación.

```{r}
sig.var<- summary(m5)$coeff[-1,4] <0.01
names(sig.var)[sig.var == TRUE]
```
...
```{r}
predicciones5 <- ifelse(test = m5$fitted.values > 0.5, yes = 1, no = 0)
matriz_confusion5 <- table(m5$model$y, predicciones3,
                          dnn = c("observaciones", "predicciones"))
matriz_confusion5
```
```{r}
TN1 <- matriz_confusion5[1,1]
FP1 <- matriz_confusion5[1,2]
FN1 <- matriz_confusion5[2,1]
TP1 <- matriz_confusion5[2,2]
```
...
accuracy11 <- (TP1+TN1)/(TP1+TN1+FP1+FN1)
tabla2 <- matrix(data = c(precision1,recall1,F11,accuracy11), ncol=1, nrow=4)
rownames(tabla2) <- c("Precision","Recall","F1","Accuracy")
tabla2
```
...
``` 
```{r}
pred5<- predict.glm(m5,newdata = data_test, type="response")
result5<- table(data_test$y, floor(pred3+0.5))
result5
```
...
```{r}
ROC5 <- roc(data_test$y, pred5)
plot(ROC5, col = "red")
auc(ROC5)
```

El AUC tiene un valor muy adecuado, aunque menor que el modelo completo. Al hacer el modelo incluyendo la duración de la llamada, se obtienen sobreestimaciones del AUC y una infraestimación del AIC, indicando mejor ajuste del esperado.
...
*Medidas de ajuste del modelo 1 (completo) y modelo 3(variables significativas)*
...
#m3
dev11 <- m4$deviance
nullDev11<- m4$null.deviance
modelChi11 <- nullDev11 - dev11
modelChi11
chigl11 <- m4$df.null - m4$df.residual
chisq.prob11 <- 1 - pchisq(modelChi11, chigl11)
chisq.prob11
R2.hl11 <- modelChi11/m4$null.deviance
R2.hl11
#m4
dev12 <- m5$deviance
nullDev12<- m5$null.deviance
modelChi12 <- nullDev12 - dev12
modelChi12
chigl12 <- m5$df.null - m5$df.residual
chisq.prob12 <- 1 - pchisq(modelChi12, chigl12)
chisq.prob12
R2.hl12 <- modelChi12/m5$null.deviance
R2.hl12
```
#Tabla recopilación de modelos
```{r}
AIC1 <- c(AIC(m1), AIC(m3), AIC(m4), AIC(m5))
BIC1 <- c(BIC(m1), BIC(m3), BIC(m4), BIC(m5))
R21 <- c(R2.hl, R2.hl1, R2.hl11, R2.hl12)
tabla2 <- matrix(data = c(AIC1,BIC1,R21), ncol=3, nrow=4)
colnames(tabla2) <- c("AIC","BIC","R2")
rownames(tabla2) <- c("Modelo 1: completo", "Modelo 3: edad", "Modelo 4: tasa de empleo", "Modelo 5: variables sign.")
tabla2
```
