...
```
En estimaciones de valores perdidos, dentro de las estimaciones univariantes, encontramos varios métodos de imputación, como el árbol de regresione o la media y la moda. 
...
Esta binarizacion de la variable pdays se convertirá en la variable pdays_aux. Transformamos la variable numerica pdays a variable binaria que toma valores {0,1}. 
...
Las variables relevantes para obtener el modelo con mejor ajuste y redimiento obtenidas por el método forward serían mes (abril, mayo y noviembre), duración de la llamada, días previos, resultado de la campaña previa, índice de confianza del consumidor y número de empleados.
...
```
Las variables relevantes para obtener el modelo con mejor ajuste y redimiento obtenidas por el método backward serían mes (abril, mayo, junio y octubre), duración de la llamada, resultado de la campaña previa, la ratio de variación del empleo y índice de confianza del consumidor. 
...
m1.1 <- glm(y ~ -duration, data = data_train, family = "binomial")
```

El modelo 1 corresponde al modelo con todas las variables.
El modelo 1.1 corresponde al modelo quitando la variable duración.
...
Los modelos 2 y 3 son modelos univariantes con la variable que menos correlaciona (edad) y la que más correlaciona con el resto (tasa de variación de empleo).
...
```{r}
m8 <- glm(y ~ job+education+default+contact+month+campaign+pdays_aux+poutcome+emp.var.rate+cons.price.idx+cons.conf.idx+nr.employed, family = binomial, data= data_train)
summary(m8)
#Método manual
m9 <- glm(y ~ month + pdays_aux + poutcome + euribor3m, data = data_train, family = binomial)
```
El modelo 8m8 <- glm(y ~ job+education+default+contact+month+campaign+pdays+poutcome+emp.var.rate+cons.price.idx+cons.conf.idx+nr.employed, family = binomial, data= data_train)
summary(m8)
#Método manual
m9 <- glm(y ~ month + pdays_aux + poutcome + euribor3m, 
...
El modelo 1 tiene valores muy altos de colinealidad, coherente con el uso de variables muy correlacionadas como mencionamos previamente. Los valores de leverage son adecuados. 

El modelo 1.1 muestra valores de homogeneidad de la varianza y de residuos inadecuados. 
...
check_model(m9)
```
Los modelos 5,6 y 7 muestran valores bajos de colinealidad, frente al último que tiene valores muy altos, aumentando el riesgo de sesgo. La normalidad en el gráfico Q-Q son similares entre modelos, siendo la más diferente en el modelo 5. Aunque los valores de leverage son altos, el único modelo en el que se ajusta es el modelo 8. El modelo 9 muestra unos valores más ajustados, tanto de colinealidad, como de leverage (dentro del intervalo de la distancia D de Cook) y la normalidad de los residuos no es adecuada, especialmente en los cuartiles intermedios.

```{r}
compare_performance(m1,m1.1,m2,m3,m4,m5,m6,m7,m8,m9)
``` 

Los modelos muestran valores altos de AIC y BIC en general, siendo índices de ajuste de parsimonia, sancionando modelos que requieren un mayor número de variables. Los menores valores son del modelo 1, pero incluye todas las variables (también la variable duración, una variable que no será útil puesto que la obtenemos tras la realización de la llamada). Lo mismo ocurre con el modelo 6, obteniendo valores ligeramente más bajos de AIC y BIC, y de RMSE, así como de $R^2$ de Tjur o el coeficiente D de determinación para variables binarias  El modelo 8 será el seleccionado, debido a la información extra y al aumento del valor de $R^2$ de Tjur y menor valor de RMsE.



```{r}
plot(compare_performance(m1,m1.1,m2,m3,m4,m5,m6,m7,m8,m9, rank = TRUE))
```
...
```
El lift chart muestra la efectividad del modelo por deciles, estando mejor ajustado en los primeros tres deciles. El KS chart muestra una estructura similar a la curva ROC.
...
```
El lift chart muestra mejor ajuste en los primeros dos deciles. El KS se ha reducido significativamente frente al modelo completo.
...
Los modelos de regresión logística muestran un ajuste pobre, quedando claro que la variable duración aumenta articialmente el ajuste. En general, el modelo completo (m1) es el modelo que ajusta mejor, pero habiendo un. 
