...
tabla<-table(data_train$y,data_train$job)
chisq.test(tabla)
```
...
tabla<-table(data_train$y,data_train$marital)
chisq.test(tabla)
```
...
```{r test de Fisher}
tabla<-table(data_train$y,data_train$education)
...
```{r test de Fisher 2}
tabla<-table(data_train$y,data_train$default)
#Con chi-cuadrado daba problemas por lo que uso fisher
fisher.test(tabla)
```
Rechazamos la hipótesis nula, por lo tanto, la variable default presenta una asociación significativa para la clase y.
...
tabla<-table(data_train$y,data_train$housing)
chisq.test(tabla)
...
tabla<-table(data_train$y,data_train$loan)
chisq.test(tabla)
chisq.test(tabla, simulate.p.value = TRUE, B = 5000)
```
...
tabla<-table(data_train$y,data_train$contact)
chisq.test(tabla)
```
...
tabla<-table(data_train$y,data_train$month)
chisq.test(tabla)
```
...
tabla<-table(data_train$y,data_train$day_of_week)
chisq.test(tabla)
```
...
tabla<-table(data_train$y,data_train$pdays_aux)
chisq.test(tabla)
```
...
tabla<-table(data_train$y,data_train$poutcome)
chisq.test(tabla)
```
...
t.test(data_train$age~data_train$y)
biserial.cor(x = data_train$age,  y = data_train$y, level = 2)
```
...
t.test(data_train$duration~data_train$y)
```
...
Para comprobar si es cierto que existen evidencias significativas realizamos el test de Student.
```{r}
t.test(data_train$campaign~data_train$y)
```
...
Para comprobar si es cierto que existen evidencias significativas realizamos el test de Student.
```{r}
t.test(data_train$previous~data_train$y)
```
...
Para comprobar si es cierto que existen evidencias significativas realizamos el test de Student.
```{r}
t.test(data_train$emp.var.rate~data_train$y)
```
...
```{r}
t.test(data_train$cons.price.idx~data_train$y)
```
...
Para comprobar si es cierto que existen evidencias significativas realizamos el test de Student.
```{r}
t.test(data_train$cons.conf.idx~data_train$y)
```
...
Para comprobar si es cierto que existen evidencias significativas realizamos el test de Student
```{r}
t.test(data_train$euribor3m~data_train$y)
```
...
Para comprobar si es cierto que existen evidencias significativas realizamos el test de Student.
```{r}
t.test(data_train$nr.employed~data_train$y)
```
...
Las variables relevantes serían número de empleados, meses, resultado de la campaña previa, vía de contacto, día de la semana, trabajo, días desde el contacto previo (dicotomizada), indice de confianza del consumidor, número de contactos durante esta campaña,  impago, ratio de variación de empleo e índice de precios al consumo.


```{r}
reg1 <- glm(formula = y ~ nr.employed + month + poutcome + contact + 
    day_of_week + job + pdays_aux + cons.conf.idx + campaign + 
    default + emp.var.rate + cons.price.idx, family = "binomial", 
    data = data_train2)
```
...
Las variables relevantes para obtener el modelo con mejor ajuste y redimiento obtenidas por el método forward serían contacto telefónico, mes (mayo, julio y septiembre), resultado de la campaña previa, índice de precios al consumo, índice de confianza del consumidor, euribor y número de empleados y días de contacto previo (dicotomizada).
...
```
Las variables relevantes para obtener el modelo con mejor ajuste y redimiento obtenidas por el método backward serían mes (abril, mayo, junio, julio y noviembre), resultado de la campaña previa, la ratio de variación del empleo, índice de precios al consumo y el euribor. 

Ambos métodos sugieren como variables relevantes comunes el mes, el resultado de la campaña previa, el índice de precios al consumo y el euribor.
...
#modelo forward
m4 <- glm(y ~ contact+month+poutcome+cons.conf.idx+cons.price.idx+euribor3m+nr.employed+pdays_aux, data = data_train2, family = "binomial")
#modelo backward
...
m6 <- glm(y ~ month+poutcome+cons.price.idx+euribor3m, data = data_train2, family = "binomial")
```
...
check_model(m8)
```
Los modelos 4,5,7 y 8 muestran valores altos de colinealidad, aumentando el riesgo de sesgo. Frente a estos, el modelo 6 muestra valores bajos de colinealidad. Se observa un mejor ajuste a la distancia D de Cook en leverage, así como un gráfico Q-Q mejor que los modelos previos. Sigue habiendo una amplia distorsión en los cuartiles centrales. 

```{r}
compare_performance(reg1,m1,m2,m3,m4,m5,m6,m7,m8)
``` 

Los modelos muestran valores altos de AIC y BIC en general, siendo índices de ajuste de parsimonia, sancionando modelos que requieren un mayor número de variables. Todos los modelos (menos el 2 y el 3, que corresponden a una sola variable predictora) muestran ajustes similares, especialmente el m1, reg1 y el m7. El mejor es el modelo reg1, aunque finalmente escogeremos el m7, que recoge las variables significativas del modelo 1. 

No se escoge el modelo reg1 porque requiere de un mayor número de variables. Este modelo tiene un AIC y un BIC semejante al modelo 1, además de un valor ligeramente superior al resto de $R^2$ de Tjur y mismo valor de RMSE, que indica el error cuadrático medio.
